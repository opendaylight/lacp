...
public class RSMThread implements Runnable
{
				...
				if (lacpPort.slaveGetPortId() == portId && 
					...
					try {
						...
						if(tmExpiryMsg.getTimerWheelType() == Utils.timerWheeltype.CURRENT_WHILE_TIMER){
                                                        RxContext objRxContext = null;
                                                        objRxContext = lacpPort.getRxContextObject();
                                                        if(objRxContext != null){
                                                                if(objRxContext.getState().getStateFlag() == LacpConst.RX_STATES.RX_DEFAULTED){
    ...
    public void portCleanupInDefaultedState(long switchId, short portId)
    {
        ...
        if(myBond != null && myBond.bondNumMembersInSw(switchId) == 0){
                if (key!=0)
                {
