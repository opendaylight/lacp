...
public class LacpGroupTblTest {
	...
	public void testLacpAddUpdateRemoveGroupPort() {
		NodeConnectorRef nodeConnectorRef1 = getNodeConnectorRef(1,1);
		...
		//Creates a Group
		Group origGroup = lacpGroupTbl.lacpAddGroup(isUnicastGrp, nodeConnectorRef1, groupId);
		assertTrue(isPortPresent(nodeConnectorRef1,origGroup));
		
		NodeConnectorRef nodeConnectorRef2 = getNodeConnectorRef(1,2);
		
		//Add a port
		Group grp = lacpGroupTbl.lacpAddPort(isUnicastGrp, nodeConnectorRef2, origGroup);
		assertTrue(isPortPresent(nodeConnectorRef2,grp));
		
		//Removes the added port
		grp = lacpGroupTbl.lacpRemPort(grp, nodeConnectorRef2, isUnicastGrp);
		assertFalse(isPortPresent(nodeConnectorRef2,grp));
		
		NodeConnectorRef nodeConnectorRef3 = getNodeConnectorRef(1,3);
		
		//Adds a port
		grp = lacpGroupTbl.lacpAddPort(isUnicastGrp, nodeConnectorRef2, origGroup);
		assertTrue(isPortPresent(nodeConnectorRef2,grp));
		assertFalse(isPortPresent(nodeConnectorRef3,grp));
		
		//Trying to Remove a port that was never added
		grp = lacpGroupTbl.lacpRemPort(grp, nodeConnectorRef3, isUnicastGrp);
		assertTrue(isPortPresent(nodeConnectorRef1,grp));
		assertTrue(isPortPresent(nodeConnectorRef2,grp));
		assertFalse(isPortPresent(nodeConnectorRef3,grp));
		
		//Checks all RemoveGroup possibilities
		
		//salService.removeGroup throws ExecutionException
		lacpGroupTbl.lacpRemGroup(isUnicastGrp, nodeConnectorRef1, groupId);
		
		//salService.removeGroup() returns unsuccessful RPCResult
		lacpGroupTbl.lacpRemGroup(isUnicastGrp, nodeConnectorRef1, groupId);
		
		//salService.removeGroup() returns successful RPCResult
		lacpGroupTbl.lacpRemGroup(isUnicastGrp, nodeConnectorRef1, groupId);

		Mockito.verify(salGroupService,Mockito.times(3)).removeGroup(any(RemoveGroupInput.class));
		
	}
	private NodeConnectorRef getNodeConnectorRef(int n,int nc){

        InstanceIdentifier<NodeConnector> nodeId = InstanceIdentifier.builder(Nodes.class)
        		.child(Node.class, new NodeKey(new NodeId("openflow:"+n)))
        		.child(NodeConnector.class, new NodeConnectorKey(new NodeConnectorId("OpenFlow_Con:"+nc))).build();
