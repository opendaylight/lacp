...
public class LacpPortTest {
   ...
   @Before
	...
	public void testSlavePortPriorityChange() {
		lacpPort.slavePortPriorityChange(lacpPort.getActorPortPriority() + 2);
		lacpPort.slavePortPriorityChange(lacpPort.getActorPortPriority());
	}

	@Test
	public void testSlaveSystemPriorityChange() {
		lacpPort.slaveSystemPriorityChange(lacpPort.getActorSystemPriority() + 2);
	...
	public void testEnableCollectingDistributing() {
		LacpAggregator agg = Mockito.mock(LacpAggregator.class);
		lacpPort.portSetAggregator(agg);
		when(agg.aggGetBond()).thenReturn(bond);
		when(agg.getIsActive()).thenReturn((short)1);
		lacpPort.enableCollectingDistributing((short)11, agg);
		when(agg.getIsActive()).thenReturn((short)0);
		lacpPort.enableCollectingDistributing((short)11, agg);
	}
	...
	/*public void testUpdateNCLacpInfo() {
		...
		//doNothing().when(wtTrn).put(LogicalDatastoreType.OPERATIONAL, any(InstanceIdentifier.class), any(LacpNodeConnector.class));
	...
	@Test
	public void testGetLacpNode() {
		//assertNotNull(lacpPort.getLacpNode());
	}
