...
public class PeriodicTxPeriodicStateTest {
	LacpPort port;
	PeriodicTxContext context;
	PeriodicTxPeriodicState periodicTxState;
...
//		 bpdu = Mockito.mock(LacpBpduInfo.class);
  //              port = Mockito.mock(LacpPort.class);
		...
		periodicTxState = new PeriodicTxPeriodicState();
...
//		periodicTxState.executeStateAction(context, port, bpdu);
	...
	@Test
	public void testPeriodicTxPeriodicState() throws Exception {
		assertEquals(LacpConst.PERIODIC_STATES.PERIODIC_TX, periodicTxState.getStateFlag());
	}
}
